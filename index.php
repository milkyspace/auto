<?php require $_SERVER['DOCUMENT_ROOT'] . '/bitrix/header.php'; ?>
<?php $APPLICATION->SetTitle("Главная"); ?>
<main class="main">
    <div class="background">
        <!-- First -->
        <section class="first">
            <div class="container">
                <div class="first_girl">
                    <img class="girl_pic" src="/local/asset/img/svg/girl.svg">

                    <a href="#calculator" class="button_calc anchor-scroll">
                        <div>
                            <img src="/local/asset/img/svg/calculator.svg" class="button_calcend">
                        </div>
                        <div class="button_calcend">
                            <div class="calc_text">Калькулятор автокредита</div>
                            <div class="calc_text2">Рассчитайте свой ежемесячный платеж по автокредиту online</div>
                        </div>
                        <div>
                            <img src="/local/asset/img/svg/play.svg" class="button_calcplay">
                        </div>
                    </a>

                </div>
                <h1 class="first_title">Автокредит - это просто!
                </h1>
                <h2 class="first_subtitle">
                    в #CITY_NAME_PRED#
                </h2>


                <ul class="list">
                    <li class="list_el">Одобрим за 15 минут!</li>
                    <li class="list_el">На любое авто Б/у или Новое</li>
                    <li class="list_el">9.9% минимальный годовой процент!</li>

                </ul>

                <!-- bo2tton -->
                <div class="botton_green_first">
                    <a href="#modal-form" rel="modal:open" class="button_first">Отправить заявку в 1 клик<img
                                src="/local/asset/img/svg/send.svg"
                                class="button_send"></a>

                    <div class="credit">
                        <h4>Отправка заявки не обязывает
                            Вас брать кредит</h4>
                    </div>
                </div>
                <div class="container_info">
                    <div class="info">
                        <div class="mini">

                            <img src="/local/asset/img/svg/mini.svg" class="icon_mini">
                            <div>
                                Минимальный
                                годовой процент
                            </div>
                        </div>
                        <div class="pasport">

                            <img src="/local/asset/img/svg/Pasport.svg" class="icon_pasport">
                            <div>
                                Без справок о доходе
                                Только права и паспорт
                            </div>
                        </div>

                        <div class="history">

                            <img src="/local/asset/img/svg/history.svg" class="icon_history">
                            <div>
                                Одобрим с плохой
                                кредитной историей
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="button_calc-block">
                <a href="#calculator" class="mobile button_calc anchor-scroll">
                    <div>
                        <img src="/local/asset/img/svg/calculator.svg" class="button_calcend">
                    </div>
                    <div class="button_calcend">
                        <div class="calc_text">Калькулятор автокредита</div>
                        <div class="calc_text2">Рассчитайте свой ежемесячный платеж по автокредиту online</div>
                    </div>
                    <div>
                        <img src="/local/asset/img/svg/play.svg" class="button_calcplay">
                    </div>
                </a>
            </div>

        </section>

        <!-- Second -->
        <section class="second">
            <div class="container">
                <div class="line_text">
                    <h1 class="second_sd ">СД-АВТО</h1>
                    <p class="second_sd_text second_sd_text_marg ">новый цивилизованный способ безопасной покупки
                        подержанного
                        или нового автомобиля.
                    </p>
                </div>
                <div class="second_cards">
                    <div class="second_card1">

                        <p class="second_card1_text"> Каждый автомобиль проходит полную техническую диагностику и имеет
                            диагностическую карту.</p>
                        <img src="/local/asset/img/svg/card1.svg" alt="" style="float: right;
									margin-top: -18px">

                    </div>

                    <div class="second_card2">


                        <p class="second_card2_text"> 100% гарантия юридической чистоты и технической исправности
                            каждого
                            автомобиля.
                        </p>
                        <img src="/local/asset/img/svg/card2.svg " alt="" style="float: right;
								margin-top: -26px">

                    </div>

                    <div class="second_card2">

                        <p class="second_card3_text">Предоставляем Автокредит через автосалон на любое новое или
                            подержанное
                            авто. Вы можете выбрать любую машину от любого продавца, не обязательно из салона.

                        </p>
                        <img src="/local/asset/img/svg/card3.svg " alt="" style="float: right;
								margin-top: -45px">


                    </div>

                    <div class="second_card4">

                        <p class="second_card4_text">Автокредит по госпрограмме в 2019 году. Госпрограмма автокредит на
                            первый
                            автомобиль. Программа семейный автокредит в 2019 году

                        </p>
                        <img src="/local/asset/img/svg/card_4.svg" alt="" style="float: right;
								margin-top: -62px">
                    </div>


                    <div class="second_card5">
                        <div class="second_card5_pic">
                            <p class="second_card5_title">минимальный годовой процент</p>

                            <p class="second_card5_text">9.9% </p>
                            <img src="/local/asset/img/svg/card5.svg" alt="" style="float: right;
								margin-top: -78px">
                        </div>

                    </div>
                </div>


                <div class="car">
                    <img src="/local/asset/img/car.png" class="car_pic">
                </div>
                <div class="line_text marg_title">
                    <h1 class="second_sd second_sd1  "> Почему выгодно взять автокредит через автосалон?</h1>
                </div>
                <div class="third_cards">
                    <div class="third_card1">
                        <div class="third_card1_pic">
                            <h4 class="third_card1_title">Проще чем кредит в банке</h4>
                            <img src="/local/asset/img/svg/01.svg" alt="" class="icon_01">
                            <p class="third_card1_text">Вам больше не нужно идти в банк подтверждать свой доход и
                                собирать справки
                                для
                                получения кредита на авто, достаточно оставить заявку online и прийти в автосалон для
                                получения
                                авто.
                            </p>

                        </div>

                    </div>


                    <div class="third_card2">
                        <div class="third_card1_pic">
                            <h4 class="third_card1_title">Экономия времени</h4>
                            <img src="/local/asset/img/svg/02.svg" alt="" class="icon_02">
                            <p class="third_card1_text">Оставьте заявку на сайте и получите одобрение уже через 15
                                минут, а
                                забрать
                                авто вы сможете в любое удобное для вас время, без очереди.
                            </p>

                        </div>

                    </div>

                    <div class="third_card2">
                        <div class="third_card1_pic">

                            <h4 class="third_card1_title">Помощь в подборе авто</h4>
                            <img src="/local/asset/img/svg/03.svg" alt="" class="icon_03">
                            <p class="third_card1_text">Предлагаем бесплатно воспользоваться помощью наших специалистов
                                по
                                автоподбору
                            </p>
                            <a href="#modal-free" rel="modal:open" class="third_card1_more"> Подробнее об автоподборе
                                <img
                                        src="/local/asset/img/svg/Arrow.svg"
                                        class="arrow">
                            </a>


                        </div>


                    </div>
                </div>

                <div id="calculator" class="line_text calculator_line_desktop">
                    <h1 class="second_sd 1"> Калькулятор автокредита
                    </h1>
                    <p class="second_sd_text second_sd_text_marg">Рассчитайте свой ежемесячный платеж по автокредиту
                        online
                    </p>
                </div>
            </div>

        </section>
        <!--Calculator  -->
        <div class="container mobile_area">
            <div id="calculator_line_mobile" class="line_text">
                <h1 class="second_sd 1"> Калькулятор автокредита
                </h1>
                <p class="second_sd_text second_sd_text_marg">Рассчитайте свой ежемесячный платеж по автокредиту
                    online
                </p>
            </div>
        </div>
        <section class="calculator second container calc_desktop">

            <div class="calc_main">
                <div class="calc_over_l calc_over_margin">
                    Выберите сумму автокредита, руб.
                    <span class="calc_over_r color-gray">
								<span class="sum_credit">1 230 380</span> <span class="calc_over_rub">руб</span>
							</span>
                </div>
                <div class="line_pic">
                    <input type="text" class="js-range-slider-1" value=""/>
                    <!--						<img src="/local/asset/img/svg/line_gray1.svg" class="line_gray">-->
                    <!--						<img src="/local/asset/img/svg/line_green.svg" class="line_green">-->
                    <!--						<img src="/local/asset/img/svg/circle_line.svg" class="circle_line">-->
                </div>
                <div class="calc_under_l">
                    0
                    <span class="calc_under_r">
								2 млн
							</span>
                </div>

            </div>
            <div class="calc_main">
                <div class="calc_over_l calc_over_margin">
                    Срок кредитования, лет.
                    <span class="calc_over_r color-gray">
							<span class="term_credit">8</span> <span class="calc_over_text">лет</span>
							</span>
                </div>
                <div class="line_pic">
                    <!--						<img src="/local/asset/img/svg/line_gray1.svg" class="line_gray">-->
                    <!--						<img src="/local/asset/img/svg/line_green.svg" class="line_green">-->
                    <!--						<img src="/local/asset/img/svg/circle_line.svg" class="circle_line">-->
                    <input type="text" class="js-range-slider-2" value=""/>
                </div>
                <div class="calc_under_l">
                    0
                    <span class="calc_under_r">
								15
							</span>
                </div>

            </div>


            <div class="calc_end">
                <div class="calc_over_l calc_over_end">
                    <span class="calc_over_l-1">
                    Ваш ежемесячный платеж, руб:
                        </span>
                    <span class="calc_over_r color-green">
								<span class="calc_result">30 380</span> <span class="calc_over_rub">руб</span>
							</span>
                </div>
                <div class="calc_under_end">
                    *Рассчет ежемесячного платежа выполнен по минимальной годовой ставке 9,9%
                </div>
            </div>
        </section>
        <section class="calculator second container calc_mobile">

            <div class="calc_main">
                <div class="calc_over_l calc_over_margin">
                    Выберите сумму автокредита, руб.
                    <span class="calc_over_r color-gray">
								<span class="sum_credit">1 230 380</span> <span class="calc_over_rub">руб</span>
							</span>
                </div>
                <div class="line_pic">
                    <input type="text" id="js-range-slider-3" class="js-range-slider-3" value=""/>
                    <!--						<img src="/local/asset/img/svg/line_gray1.svg" class="line_gray">-->
                    <!--						<img src="/local/asset/img/svg/line_green.svg" class="line_green">-->
                    <!--						<img src="/local/asset/img/svg/circle_line.svg" class="circle_line">-->
                </div>
                <div class="calc_under_l">
                    0
                    <span class="calc_under_r" style="padding-left: 27px;">
								500 тыс
                    </span>
                    <span class="calc_under_r">
								1 млн
                    </span>
                    <span class="calc_under_r">
								1.5 млн
                    </span>
                    <span class="calc_under_r">
								2 млн
                    </span>
                </div>

            </div>

            <a href="" class="minus ones">-</a>
            <a href="" class="plus ones">+</a>

            <div class="calc_main pt-63">
                <div class="calc_over_l calc_over_margin">
                    Срок кредитования, лет.
                    <span class="calc_over_r color-gray">
							<span class="term_credit">8</span> <span class="calc_over_text">лет</span>
							</span>
                </div>
                <div class="line_pic">
                    <!--						<img src="/local/asset/img/svg/line_gray1.svg" class="line_gray">-->
                    <!--						<img src="/local/asset/img/svg/line_green.svg" class="line_green">-->
                    <!--						<img src="/local/asset/img/svg/circle_line.svg" class="circle_line">-->
                    <input type="text" id="js-range-slider-4" class="js-range-slider-4" value=""/>
                </div>
                <div class="calc_under_l">
                    0
                    <span class="calc_under_r" style="position:absolute; left: 60px">
								1
                    </span>
                    <span class="calc_under_r" style="position:absolute; left: 100px">
								2
                    </span>
                    <span class="calc_under_r" style="position:absolute; left: 138px;">
								3
                    </span>
                    <span class="calc_under_r" style="position:absolute; left: 176px">
								4
                    </span>
                    <span class="calc_under_r" style="position:absolute; left: 215px">
								5
                    </span>
                    <span class="calc_under_r" style="position:absolute; left: 253px">
								6
                    </span>
                    <span class="calc_under_r" style="position:absolute; left: 291px">
								7
                    </span>
                    <span class="calc_under_r" style="">
								8
                    </span>
                </div>

            </div>

            <a href="" class="minus t-290">-</a>
            <a href="" class="plus t-290">+</a>

            <div class="calc_end">
                <div class="calc_over_l calc_over_end">
                    <span class="calc_over_l-1">
                    Ваш ежемесячный платеж, руб:
                        </span>
                    <span class="calc_over_r color-green">
								<span class="calc_result">30 380</span> <span class="calc_over_rub">руб</span>
							</span>
                </div>
                <div class="calc_under_end">
                    *Рассчет ежемесячного платежа выполнен по минимальной годовой ставке 9,9%
                </div>
            </div>
        </section>
        <!-- Button -->
        <div class=" container  container_margin">
            <div class="botton_green">
                <a href="#modal-form" rel="modal:open" class="button_first button_margin ">Отправить заявку в 1 клик<img
                            src="/local/asset/img/svg/send.svg"
                            class="button_send"></a>

                <div class="credit">
                    <h4>Отправка заявки не обязывает
                        Вас брать кредит</h4>
                </div>
            </div>
            <div class="line_text ">
                <h1 class="second_sd 1"> Как получить Автокредит ?
                </h1>
                <p class="second_sd_text credit_text  second_text_margin">На любой подержанный Б/у или Новый автомобиль
                    в
                    #CITY_NAME_PRED#, не обязательно из автосалона!</p>
            </div>
        </div>
        <!-- Question 5 colomn -->

        <div class="question container desktop_question">
            <div class="question_card1">
                <img src="/local/asset/img/svg/request.svg" alt="">
                <p class="question_card1_text">Оставьте
                    заявку
                </p>

            </div>
            <div class="question_card2">
                <img src="/local/asset/img/svg/30sec.svg" alt="">
                <p class="question_card2_text">Мы перезвоним
                    за 30 секунд
                </p>

            </div>
            <div class="question_card3">
                <img src="/local/asset/img/svg/chat.svg" alt="">
                <p class="question_card3_text">Проконсультируем и ответим на все вопросы по Автокредитованию
                </p>

            </div>
            <div class="question_card4">
                <img src="/local/asset/img/svg/money.svg" alt="">
                <p class="question_card4_text">Одобрим нужную вам сумму
                </p>

            </div>
            <div class="question_card5">
                <img src="/local/asset/img/svg/smile.svg" alt="">
                <p class="question_card5_text">Заберите авто
                    в удобное время
                </p>

            </div>
        </div>

        <div class="question container mobile_question">
            <div class="question_first">
                <div class="question_card1">
                    <img src="/local/asset/img/svg/request.svg" alt="">
                    <p class="question_card1_text">Оставьте
                        заявку
                    </p>

                </div>
                <div class="question_card2">
                    <img src="/local/asset/img/svg/30sec.svg" alt="">
                    <p class="question_card2_text">Мы перезвоним
                        за 30 секунд
                    </p>

                </div>
            </div>
            <div class="question_second">
                <div class="question_card3">
                    <img src="/local/asset/img/svg/chat.svg" alt="">
                    <p class="question_card3_text">Проконсультируем и ответим на все вопросы по Автокредитованию
                    </p>

                </div>
                <div class="question_card4">
                    <img src="/local/asset/img/svg/money.svg" alt="">
                    <p class="question_card4_text">Одобрим нужную вам сумму
                    </p>

                </div>
            </div>
            <div class="question_third">
                <div class="question_card5">
                    <img src="/local/asset/img/svg/smile.svg" alt="">
                    <p class="question_card5_text">Заберите авто
                        в удобное время
                    </p>

                </div>
            </div>
        </div>
    </div>
    <!-- Request -->
    <section class="request container">
        <? $APPLICATION->IncludeComponent("pwd:connect_form", "main", []); ?>
    </section>
    <!-- Contacts -->
    <section id="contacts" class="contacts">
        <div class="container">
            <h3 class="contacts_h3">Остались вопросы?</h3>
            <h5 class="contacts_h5">Свяжитесь с нами!</h5>

            <!--contacts_info  -->
            <div class="contacts_info">

                <div>
                    <div class="contacts__address">
                        <div class="contacts__address_pic">
                            <img src="/local/asset/img/svg/geo_icon.svg" alt="" class="contacts__img">
                            <h4 class="contacts__title">Адрес</h4>
                            <p class="contacts_text">#CITY_NAME#, #CITY_ADDRESS#
                            </p>

                        </div>

                    </div>

                    <div class="contacts__time">
                        <div class="contacts__address_pic">
                            <img src="/local/asset/img/svg/time_icon.svg" alt="" class="contacts__img">
                            <h4 class="contacts__title">Режим работы</h4>

                            <p class="contacts_text">#CITY_WORKTIME#
                            </p>

                        </div>

                    </div>

                    <div class="contacts__call">
                        <div class="contacts__address_pic">
                            <img src="/local/asset/img/svg/call_icon.svg" alt="" class="contacts__img">
                            <h4 class="contacts__title">Телефон</h4>

                            <p class="contacts_text"><a href="tel:#CITY_PHONE#">#CITY_PHONE#</a>
                            </p>

                        </div>

                    </div>

                    <div class="contacts__letter">
                        <div class="contacts__address_pic">
                            <img src="/local/asset/img/svg/letter_icon.svg" alt="" class="contacts__img">
                            <h4 class="contacts__title">E-mail</h4>

                            <p class="contacts_text"><a
                                        href="mailto:#CITY_EMAIL#?subject=Автокредит">#CITY_EMAIL#</a>
                            </p>

                        </div>

                    </div>
                </div>
                <div class="map_desktop">
                    <script type="text/javascript" charset="utf-8" async
                            src="<?= $CITY->GetProperty('MAP')['VALUE'] ?>"></script>
                </div>
            </div>
            <div class="contacts_info_mobile_container">
                <div class="contacts_info contacts_info_mobile">

                    <div>
                        <div class="contacts__address">
                            <div class="contacts__address_pic">
                                <img src="/local/asset/img/svg/geo_icon.svg" alt="" class="contacts__img">
                                <h4 class="contacts__title">Адрес</h4>
                                <p class="contacts_text">#CITY_NAME#, #CITY_ADDRESS#
                                </p>

                            </div>

                        </div>

                        <div class="contacts__time">
                            <div class="contacts__address_pic">
                                <img src="/local/asset/img/svg/time_icon.svg" alt="" class="contacts__img">
                                <h4 class="contacts__title">Режим работы</h4>

                                <p class="contacts_text">#CITY_WORKTIME#
                                </p>

                            </div>

                        </div>

                        <div class="contacts__call">
                            <div class="contacts__address_pic">
                                <img src="/local/asset/img/svg/call_icon.svg" alt="" class="contacts__img">
                                <h4 class="contacts__title">Телефон</h4>

                                <p class="contacts_text"><a href="tel:#CITY_PHONE#">#CITY_PHONE#</a>
                                </p>

                            </div>

                        </div>

                        <div class="contacts__letter">
                            <div class="contacts__address_pic">
                                <img src="/local/asset/img/svg/letter_icon.svg" alt="" class="contacts__img">
                                <h4 class="contacts__title">E-mail</h4>

                                <p class="contacts_text"><a
                                            href="mailto:#CITY_EMAIL#?subject=Автокредит">#CITY_EMAIL#</a>
                                </p>

                            </div>

                        </div>
                    </div>
                    <div class="map_desktop">
                        <script type="text/javascript" charset="utf-8" async
                                src="<?= $CITY->GetProperty('MAP')['VALUE'] ?>"></script>
                    </div>
                </div>
            </div>

        </div>

    </section>
    <section class="request map_mobile">
        <script type="text/javascript" charset="utf-8" async
                src="<?= $CITY->GetProperty('MAP')['VALUE'] ?>"></script>
    </section>
    <?php require $_SERVER['DOCUMENT_ROOT'] . '/bitrix/footer.php'; ?>
